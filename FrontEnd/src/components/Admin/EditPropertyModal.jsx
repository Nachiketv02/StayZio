...
import { updateProperty } from '../../services/Admin/AdminApi';

function EditPropertyModal({ property, onClose, onUpdateProperty }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        if (!['images', 'amenities', 'existingImages'].includes(key)) {
      ...
      const existingImageIds = formData.existingImages.map(img => img._id);
      formDataToSend.append('existingImages', JSON.stringify(existingImageIds));
      
      // Add property ID
      formDataToSend.append('propertyId', property._id);

      const updatedProperty = await updateProperty(property._id, formDataToSend);
